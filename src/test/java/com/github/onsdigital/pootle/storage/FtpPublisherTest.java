package com.github.onsdigital.pootle.storage;

import com.github.onsdigital.helpers.Configuration;
...
public class FtpPublisherTest {
    ...
    public void setUp() throws IOException {
        //example json file
        String fileContents = "my test data";
        InputStream in = new ByteArrayInputStream(fileContents.getBytes(StandardCharsets.UTF_8));
        when(data.getInputStream()).thenReturn(in);
        when(data.getName()).thenReturn("data.txt");
    ...
    public void shouldPublishFile() throws IOException {
        ...
        System.out.println(files[0].getName() + " " + contents);
        ftpClient.quit();
        ftpClient.disconnect();

        assertThat(files.length, is(1));
        assertThat(files[0].getName(), is("data.txt"));
    }
